...
reduce | [any object `o`,] list `l`, function `f` | any object | Applies the `f` cumulatively to pairs of elements in `l`, to produce a final result (`f` should accept two parameters and return a value based on those two parameters). Optionally uses `o` as the initial element or as the result if the `l` is empty.
deepReduce | [any object `o`,] list `l`, function `f` | any object | Applies the `f` cumulatively to pairs of elements in `l` (and recursively traverses nested lists), to produce a final result (`f` should accept two parameters and return a value based on those two parameters). Optionally uses `o` as the initial element or as the result if the `l` is empty. Do not expect any order in the evaluation of the elements.
